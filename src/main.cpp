...
// 0=Normal, 1=Gyro Calib, 2=Claw Servo Calib, 3=Lifter Calib
enum RobotMode { MODE_NORMAL = 0, MODE_CALIB_GYRO = 1, MODE_CALIB_CLAW = 2, MODE_CALIB_LIFT = 3 };
RobotMode robotMode = MODE_NORMAL;
...
void processGamepad(ControllerPtr ctl) {
  ...
  // ─── Claw Servo Calibration Mode (MODE_CALIB_CLAW) ──────────────────────
  // Masuk: tahan Triangle+Share 3det && robotMode==MODE_NORMAL
  // Keluar: Share (tanpa Triangle) atau Triangle+Share 3det lagi
  {
    ...
    const bool calibBtn = hasCtl && ctl->y() && ctl->miscSelect(); // Triangle+Share

    // ── Masuk mode: hanya boleh dari MODE_NORMAL ──────────────────────────
    if (calibBtn && robotMode == MODE_NORMAL) {
      ...
      if (held >= 3000) {
        robotMode = MODE_CALIB_CLAW;
        debMs = millis(); holdActive = false;
        startGripperWiggle(2);
        Serial.printf("[ClawCalib] MODE ON (mode=%d)\n", robotMode);
        Serial.println("  L1=R-open++ | L2=R-open-- | R1=F-open++ | R2=F-open--");
        Serial.println("  Share=save&keluar");
      }
    } else if (!calibBtn && robotMode == MODE_NORMAL) {
      holdActive = false;
    }

    // ── Dalam mode MODE_CALIB_CLAW ────────────────────────────────────────
    if (robotMode == MODE_CALIB_CLAW && hasCtl) {
      const bool  l1b  = ctl->l1();                          // rear -1°
      const bool  l2b  = (float)ctl->brake()    / 1023.0f > 0.3f; // rear +1°
      const bool  r1b  = ctl->r1();                          // front -1°
      const bool  r2b  = (float)ctl->throttle() / 1023.0f > 0.3f; // front +1°
      const bool  xBtn = ctl->a();                           // X = save posisi ini sebagai BUKA
      const bool  oBtn = ctl->b();                           // O = save posisi ini sebagai TUTUP

      // Working angle: posisi servo SEKARANG yang lagi diedit
      static float workR = -1.0f; // -1 = belum init
      static float workF = -1.0f;
      static uint8_t lastServo = 0; // 1=rear 2=front

      // Init working angle dari config saat pertama masuk
      if (workR < 0.0f) workR = cfg_claw_belakang_min;
      if (workF < 0.0f) workF = cfg_claw_depan_min;

      // Key-repeat helper: klik sekali = +1°, tahan 1det → repeat tiap 300ms
      // Tiap button punya state: prevBtn, holdStart, lastRepeat
      struct KeyRep {
        bool     prev       = false;
        uint32_t holdStart  = 0;
        uint32_t lastRepeat = 0;
        // Kembalikan true kalau harus nambah 1 derajat sekarang
        bool tick(bool cur, uint32_t now) {
          bool fire = false;
          if (cur && !prev) {          // rising edge → langsung +1
            fire = true;
            holdStart  = now;
            lastRepeat = now;
          } else if (cur && prev) {    // masih ditahan
            if (now - holdStart >= 1000 &&        // sudah 1det
                now - lastRepeat  >= 300) {        // repeat interval 300ms
              fire = true;
              lastRepeat = now;
            }
          }
      ...
      static KeyRep krL1, krL2, krR1, krR2;
      uint32_t now = millis();

      if (krL1.tick(l1b, now)) { workR = constrain(workR - 1.0f, 0.0f, 180.0f); lastServo = 1; servoClawRear.setAngle(workR); }
      if (krL2.tick(l2b, now)) { workR = constrain(workR + 1.0f, 0.0f, 180.0f); lastServo = 1; servoClawRear.setAngle(workR); }
      if (krR1.tick(r1b, now)) { workF = constrain(workF - 1.0f, 0.0f, 180.0f); lastServo = 2; servoClawFront.setAngle(workF); }
      if (krR2.tick(r2b, now)) { workF = constrain(workF + 1.0f, 0.0f, 180.0f); lastServo = 2; servoClawFront.setAngle(workF); }

      // X = save posisi sekarang sebagai BUKA (open/min) servo terakhir
      static bool lastX = false;
      if (xBtn && !lastX && lastServo != 0) {
        preferences.begin("config", false);
        if (lastServo == 1) {
          cfg_claw_belakang_min = workR;
          preferences.putFloat("claw_r_min", workR);
          Serial.printf("[ClawCalib] Belakang BUKA saved: %.0f\n", workR);
        } else {
          cfg_claw_depan_min = workF;
          preferences.putFloat("claw_f_min", workF);
          Serial.printf("[ClawCalib] Depan BUKA saved: %.0f\n", workF);
        }
        preferences.end();
      }
      lastX = xBtn;

      // O = save posisi sekarang sebagai TUTUP (close/max) servo terakhir
      static bool lastO = false;
      if (oBtn && !lastO && lastServo != 0) {
        preferences.begin("config", false);
        if (lastServo == 1) {
          cfg_claw_belakang_max = workR;
          preferences.putFloat("claw_r_max", workR);
          Serial.printf("[ClawCalib] Belakang TUTUP saved: %.0f\n", workR);
        } else {
          cfg_claw_depan_max = workF;
          preferences.putFloat("claw_f_max", workF);
          Serial.printf("[ClawCalib] Depan TUTUP saved: %.0f\n", workF);
        }
        preferences.end();
      }
      lastO = oBtn;
      ...
      if (millis() - printMs > 300) {
        printMs = millis();
        Serial.printf("[ClawCalib] [%s] workR=%.0f workF=%.0f | saved F:o=%.0f c=%.0f R:o=%.0f c=%.0f\n",
      ...
      // Triangle+Share 3det = EXIT + auto-save semua
      if (calibBtn) {
        if (!holdActive) { holdActive = true; holdStartMs = millis(); }
        uint32_t held = millis() - holdStartMs;
        if (held >= 1000 && held < 1100) Serial.println("[ClawCalib] Tahan 2det lagi...");
        if (held >= 2000 && held < 2100) Serial.println("[ClawCalib] Tahan 1det lagi...");
        if (held >= 3000 && millis() - debMs > 500) {
          updateGripperConfigs();
          saveConfigurations();
          workR = -1.0f; workF = -1.0f; // reset untuk sesi berikutnya
          robotMode = MODE_NORMAL; debMs = millis(); holdActive = false;
          startGripperWiggle(3);
          Serial.printf("[ClawCalib] EXIT+SAVED\n");
        }
  ...
  // ─── Lifter Calibration Mode (MODE_CALIB_LIFT) ────────────────────────────
  // Toggle: Square+Share ditahan 3 detik (hanya dari MODE_NORMAL)
  // L1=rear-UP++ | L2=rear-DOWN++ | R1=front-UP++ | R2=front-DOWN++
  // X=save UP | O=save DOWN | Share=save&keluar
  {
    static uint32_t debMs       = 0;
    static uint32_t tickMs      = 0;
    static uint32_t printMs     = 0;
    ...
    const bool hasCtl   = ctl && ctl->isConnected();
    const bool calibBtn = hasCtl && ctl->x() && ctl->miscSelect(); // Square+Share

    // ── Masuk: hanya dari MODE_NORMAL ────────────────────────────────────
    if (calibBtn && robotMode == MODE_NORMAL) {
      if (!holdActive) { holdActive = true; holdStartMs = millis(); }
      uint32_t held = millis() - holdStartMs;
      if (held >= 1000 && held < 1100) Serial.println("[LiftCalib] Tahan Square+Share... 2 detik lagi");
      if (held >= 2000 && held < 2100) Serial.println("[LiftCalib] Tahan Square+Share... 1 detik lagi");
      if (held >= 3000) {
        robotMode = MODE_CALIB_LIFT;
        debMs = millis(); holdActive = false;
        startGripperWiggle(2);
        Serial.printf("[LiftCalib] MODE ON (mode=%d)\n", robotMode);
        Serial.println("  L1=rear-UP++ | L2=rear-DOWN++ | R1=front-UP++ | R2=front-DOWN++");
        Serial.println("  X=save-UP | O=save-DOWN | Share=save&keluar");
      }
    } else if (!calibBtn && robotMode == MODE_NORMAL) {
      holdActive = false;
    }

    // ── Dalam MODE_CALIB_LIFT ─────────────────────────────────────────────
    if (robotMode == MODE_CALIB_LIFT && hasCtl) {
      const bool l1b  = ctl->l1();                                    // rear -1°
      const bool l2b  = (float)ctl->brake()    / 1023.0f > 0.3f;     // rear +1°
      const bool r1b  = ctl->r1();                                    // front -1°
      const bool r2b  = (float)ctl->throttle() / 1023.0f > 0.3f;     // front +1°
      const bool xBtn = ctl->a();   // X = save posisi ini sebagai UP
      const bool oBtn = ctl->b();   // O = save posisi ini sebagai DOWN

      // Working angle per lifter
      static float workR = -1.0f;
      static float workF = -1.0f;
      static uint8_t lastLift = 0; // 1=rear 2=front

      if (workR < 0.0f) workR = cfg_lift_belakang_min;
      if (workF < 0.0f) workF = cfg_lift_depan_min;

      // Key-repeat: klik=1°, tahan 1det → repeat tiap 300ms
      struct KeyRep {
        bool prev=false; uint32_t holdStart=0, lastRepeat=0;
        bool tick(bool cur, uint32_t now) {
          bool fire=false;
          if (cur && !prev)          { fire=true; holdStart=lastRepeat=now; }
          else if (cur && prev && now-holdStart>=1000 && now-lastRepeat>=300) { fire=true; lastRepeat=now; }
      ...
      if (krL1.tick(l1b, now)) { workR = constrain(workR - 1.0f, 0.0f, 180.0f); lastLift = 1; servoLiftRear.setAngle(workR); }
      if (krL2.tick(l2b, now)) { workR = constrain(workR + 1.0f, 0.0f, 180.0f); lastLift = 1; servoLiftRear.setAngle(workR); }
      if (krR1.tick(r1b, now)) { workF = constrain(workF - 1.0f, 0.0f, 180.0f); lastLift = 2; servoLiftFront.setAngle(workF); }
      if (krR2.tick(r2b, now)) { workF = constrain(workF + 1.0f, 0.0f, 180.0f); lastLift = 2; servoLiftFront.setAngle(workF); }

      // X = save posisi sekarang sebagai UP (min)
      static bool lastX = false;
      if (xBtn && !lastX && lastLift != 0) {
        preferences.begin("config", false);
        if (lastLift == 1) { cfg_lift_belakang_min = workR; preferences.putFloat("lift_r_min", workR); Serial.printf("[LiftCalib] Belakang UP saved: %.0f\n", workR); }
        else               { cfg_lift_depan_min    = workF; preferences.putFloat("lift_f_min", workF); Serial.printf("[LiftCalib] Depan UP saved: %.0f\n",    workF); }
        preferences.end();
      }
      lastX = xBtn;

      // O = save posisi sekarang sebagai DOWN (max)
      static bool lastO = false;
      if (oBtn && !lastO && lastLift != 0) {
        preferences.begin("config", false);
        if (lastLift == 1) { cfg_lift_belakang_max = workR; preferences.putFloat("lift_r_max", workR); Serial.printf("[LiftCalib] Belakang DOWN saved: %.0f\n", workR); }
        else               { cfg_lift_depan_max    = workF; preferences.putFloat("lift_f_max", workF); Serial.printf("[LiftCalib] Depan DOWN saved: %.0f\n",    workF); }
        preferences.end();
      }
      lastO = oBtn;
      ...
      if (millis() - printMs > 300) {
        printMs = millis();
        Serial.printf("[LiftCalib] [%s] workR=%.0f workF=%.0f | saved F:up=%.0f dn=%.0f R:up=%.0f dn=%.0f\n",
                      lastLift==1?"R*":lastLift==2?"F*":"--",
                      workR, workF,
                      cfg_lift_depan_min, cfg_lift_depan_max,
                      cfg_lift_belakang_min, cfg_lift_belakang_max);
      }

      // Square+Share 3det = EXIT + auto-save semua (satu-satunya cara keluar)
      if (calibBtn) {
        if (!holdActive) { holdActive = true; holdStartMs = millis(); }
        uint32_t held = millis() - holdStartMs;
        if (held >= 1000 && held < 1100) Serial.println("[LiftCalib] Tahan 2det lagi...");
        if (held >= 2000 && held < 2100) Serial.println("[LiftCalib] Tahan 1det lagi...");
        if (held >= 3000 && millis() - debMs > 500) {
          updateGripperConfigs();
          saveConfigurations();
